...
public final class VaultHook extends AbstractHook {
  ...
  public VaultHook(AbstractEconomyPlugin plugin) {
    super(plugin);
  }
  ...
  @Override
  public void load() {
    service = new VaultService(getPlugin().getEconomyPlatform());
    Bukkit.getServicesManager().register(Economy.class, service, getPlugin(), ServicePriority.Highest);
    Collection<RegisteredServiceProvider<Economy>> providers = Bukkit.getServicesManager().getRegistrations(Economy.class);

    if (providers.isEmpty() || providers.stream().noneMatch(service -> service.getProvider() == this.service))
      getLogger().severe("Failed to register with net.milkbowl.vault.economy.Economy.class");

    if (providers.size() > 1)
      getLogger().warning("Multiple economy plugins detected! This may cause issues with other plugins.");
  }

  @Override
  public void close() {
    Bukkit.getServicesManager().unregister(Economy.class, service);
  }

  private static class VaultService implements Economy {

    private final EconomyPlatform platform;
    private final Currency currency;

    public VaultService(EconomyPlatform platform) {
